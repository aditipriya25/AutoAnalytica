...
const Plot = dynamic(() => import("react-plotly.js"), {
  ssr: false,
  loading: () => (
    <div className="animate-pulse bg-white/5 w-full h-64 rounded-xl flex items-center justify-center text-slate-500">
      Initializing Interactive Visuals...
    </div>
  ),
});
...
export default function ReportsPage() {
  ...
  if (eda?.statistical_summary) {
    const stats = Object.entries(eda.statistical_summary)
      .filter(([, s]: any) => typeof s.mean === "number")
      .slice(0, 8);
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in duration-500 max-w-6xl mx-auto w-full">
      ...
      <div className="flex justify-between items-end border-b border-blue-500/20 pb-4">
        <div>
          ...
          <p className="text-slate-400 mt-2">
            Auto-generated Tableau-style Monolithic Report bridging core
        ...
        <div className="flex gap-3">
          <Link
            ...
            className="flex items-center gap-2 px-4 py-2 rounded border border-blue-500/50 text-blue-400 font-bold hover:bg-blue-500/10 transition"
          >
            <FileText className="w-4 h-4" /> Deep-Dive Document
          </Link>
          <button
            onClick={executePrint}
            className="flex items-center gap-2 px-4 py-2 rounded bg-blue-500 text-white font-bold hover:bg-slate-300 hover:shadow-lg transition"
          >
      ...
      <div
        ...
        className="flex flex-col gap-6 pt-2 pb-6 px-1 rounded-2xl bg-[#020617]"
      >
        {/* KPI Row */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <div className="glass-panel-cyber p-6 rounded-xl flex flex-col gap-1">
            <span className="text-slate-300 text-sm font-bold uppercase tracking-wider flex items-center gap-2">
              <Database className="w-4 h-4" /> Absolute Rows
            </span>
            <span className="text-3xl font-bold text-white">
              {eda?.overview?.rows?.toLocaleString() || "N/A"}
            </span>
          </div>
          <div className="glass-panel-cyber p-6 rounded-xl flex flex-col gap-1">
            <span className="text-slate-300 text-sm font-bold uppercase tracking-wider flex items-center gap-2">
              <TableProperties className="w-4 h-4" /> Features
            </span>
            <span className="text-3xl font-bold text-white">
              {eda?.overview?.columns?.toLocaleString() || "N/A"}
            </span>
          </div>
          <div className="glass-panel-cyber p-6 rounded-xl flex flex-col gap-1">
            <span className="text-blue-500 text-sm font-bold uppercase tracking-wider flex items-center gap-2">
              <Sigma className="w-4 h-4" /> Memory Weight
            </span>
            <span className="text-3xl font-bold text-white">
              {eda?.overview?.memory_usage_kb?.toLocaleString() || "0"} KB
            </span>
          </div>
          <div className="glass-panel-cyber p-6 rounded-xl flex flex-col gap-1">
            <span className="text-blue-500 text-sm font-bold uppercase tracking-wider flex items-center gap-2">
              <Activity className="w-4 h-4" /> Schema Integrity
            </span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="glass-panel-cyber p-6 rounded-xl md:col-span-1 flex flex-col min-h-[400px]">
            <h3 className="text-lg font-bold mb-4 border-b border-blue-500/10 pb-2">
              Active Data Schema
            </h3>
            <div className="flex-1 flex flex-col gap-2 overflow-y-auto max-h-[400px] pr-2">
              {eda?.schema ? (
                eda.schema.map((c: any) => (
                  <div
                    key={c.name}
                  ...
                  >
                    ...
                    <span className="text-xs bg-slate-300/20 text-slate-300 px-2 py-1 rounded">
                      {c.type}
                    </span>
                  </div>
                ))
              ...
              <table className="w-full text-left text-sm whitespace-nowrap">
                ...
                <tbody className="divide-y divide-white/5">
                  {eda?.statistical_summary
                    ? Object.entries(eda.statistical_summary)
                        .slice(0, 6)
                        .map(([col, s]: [string, any]) => (
                          <tr key={col} className="hover:bg-white/5">
                            <td className="py-3 pr-4 font-mono text-white truncate max-w-[120px]">
                              {col}
                            </td>
                            ...
                            <td className="py-3 pr-4 text-slate-300 font-bold">
                              {s.max.toFixed(2)}
                            </td>
              ...
              <div className="mt-6 w-full flex-1 min-h-[250px] border border-white/5 rounded-lg overflow-hidden bg-black/20 relative">
                {plotData.length > 0 ? (
                  ...
                  <div className="flex items-center justify-center h-full text-slate-500 text-sm font-mono">
                    Insufficient Numeric Data for Plotting
                  </div>
                )}
              </div>
              ...
              {aiReport && (
                <div className="mt-auto pt-4 flex flex-col gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500">
                  <div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-xl">
                    <h4 className="text-slate-300 font-bold mb-2 uppercase tracking-wider text-xs">
                      AI Executive Summary
                    </h4>
                    <p className="text-white text-sm leading-relaxed">
                      {aiReport.summary || aiReport.error}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 bg-black/30 rounded-xl border border-white/5">
                      <h4 className="text-slate-400 font-bold mb-2 uppercase tracking-wider text-xs border-b border-white/10 pb-2">
                        Key Findings
                      </h4>
                      <ul className="text-sm text-white space-y-2 list-disc list-inside">
                        {aiReport.key_findings?.map(
                          (f: string, i: number) => (
                            <li key={i}>{f}</li>
                          )
                        )}
